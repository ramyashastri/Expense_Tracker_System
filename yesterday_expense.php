  ...
  <tbody>
    <?php
        ...
        if($db_user_name=='admin'){
            $fetch_expense="SELECT * FROM records WHERE item_date='$yesterday_date' ORDER BY item_date DESC";
        }
        else{
        $fetch_expense="SELECT * FROM records WHERE item_date='$yesterday_date' AND item_id='$db_user_id' ORDER BY item_date DESC";
        }
        ...
        if(mysqli_num_rows($run_fetch_expense)>0){
            ...
            ?>          
                ...
                <td><a href="./delete_yesterday.php?del_id=<?php echo $row['id'] ?>"><button class="button me-3">Delete</button></a>
